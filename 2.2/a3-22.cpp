...
pthread_t m1,m2,m3,m4,m5,m6,m7;  // define the threads
pthread_t n1,n2,n3,n4,n5,n6,n7;
...
void decreaseSuccessorPred(string);
void flowMsg(string);
   ...
   //        break;
        ...
        for (int i=0; i<numTasks; i++)
        {
            if (myDebug)
            {
                cout << " ML in for i=" << i
                << " pred[i]=" << pred[i]
                << " taskID=" << taskId[i] << endl;
            }
            if (pred[i] <=0)
            {
                if (myDebug)
                {
                    cout << " ML in for pred <=0 taskStarted=" << taskStarted[i]
                    << " taskId[i]=" << taskId[i] << endl;
                }
...
//    int locI = inI;
    ...
    *locI = inI;
    ...
    pthread_mutex_unlock(&numTLock);


    if (inTask == "M1")
        pthread_create(&m1,NULL,runM1,&(tIdx[curIdx]));

    if (inTask == "M2")
        pthread_create(&m2,NULL,runM2,&(tIdx[curIdx]));

    if (inTask == "M3")
        pthread_create(&m3,NULL,runM3,&(tIdx[curIdx]));

    if (inTask == "M4")
        pthread_create(&m4,NULL,runM4,&(tIdx[curIdx]));

    if (inTask == "M5")
        pthread_create(&m5,NULL,runM5,&(tIdx[curIdx]));

    if (inTask == "M6")
        pthread_create(&m6,NULL,runM6,&(tIdx[curIdx]));

    if (inTask == "M7")
        pthread_create(&m7,NULL,runM7,&(tIdx[curIdx]));

    if (inTask == "N1")
        pthread_create(&n1,NULL,runN1,&(tIdx[curIdx]));

    if (inTask == "N2")
        pthread_create(&n2,NULL,runN2,&(tIdx[curIdx]));

    if (inTask == "N3")
        pthread_create(&n3,NULL,runN3,&(tIdx[curIdx]));

    if (inTask == "N4")
        pthread_create(&n4,NULL,runN4,&(tIdx[curIdx]));

    if (inTask == "N5")
        pthread_create(&n5,NULL,runN5,&(tIdx[curIdx]));

    if (inTask == "N6")
        pthread_create(&n6,NULL,runN6,&(tIdx[curIdx]));

    if (inTask == "N7")
        pthread_create(&n7,NULL,runN7,&(tIdx[curIdx]));

    if (inTask == "P1")
        pthread_create(&p1,NULL,runP1,&(tIdx[curIdx]));

    if (inTask == "P2")
        pthread_create(&p2,NULL,runP2,&(tIdx[curIdx]));

    if (inTask == "P3")
        pthread_create(&p3,NULL,runP3,&(tIdx[curIdx]));

    if (inTask == "P4")
        pthread_create(&p4,NULL,runP4,&(tIdx[curIdx]));

    if (inTask == "P5")
        pthread_create(&p5,NULL,runP5,&(tIdx[curIdx]));

    if (inTask == "P6")
        pthread_create(&p6,NULL,runP6,&(tIdx[curIdx]));

    if (inTask == "P7")
        pthread_create(&p7,NULL,runP7,&(tIdx[curIdx]));

    if (inTask == "T1")
        pthread_create(&t1,NULL,runT1,&(tIdx[curIdx]));

    if (inTask == "T2")
        pthread_create(&t2,NULL,runT2,&(tIdx[curIdx]));

    if (inTask == "T3")
        pthread_create(&t3,NULL,runT3,&(tIdx[curIdx]));
...
void decreaseSuccessorPred(string inTask)
{
    flowMsg("In decreaseSuccessorPred");
    if (myDebug)
    {
        cout << "  in decreaseSuccessorPred  inTask=" << inTask << endl;
    }
    ...
    for (int i=0; i<numTasks; i++)
    {
        if (taskId[i] == inTask)
        {
            if (myDebug)
            {
                cout << "   decreaseSuccessorPred loop I=" << i << " inTask=" << inTask
                  << " pred[i]=" << pred[i]  << endl;
            }
	    ...
	    pthread_mutex_unlock(&pLock);
            statusDisplay("decreasing predecessor count for " + inTask);
            if (myDebug && pred[i] == -2)
            {
                cout << " in decreaseSuccessorPred pred[i] -2  inTask=" << inTask << endl;
                freopen("output22.txt","a",stdout);
            }
        }
    }

    flowMsg("   leaving decreaseSuccessorPred");
    return;

}   // end of decreasesuccessorpred
...
void checkSuccessors(int inI)
{
    ...
    {           // call routine to decrease the predecessor count
        ...
        decreaseSuccessorPred(successor[inI][i]);
    }
